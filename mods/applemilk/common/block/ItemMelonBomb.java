...
public class ItemMelonBomb extends ItemBlock{
	...
	public ItemMelonBomb(int itemId)
	{
		super(itemId);
		setMaxDamage(0);
		setHasSubtypes(true);
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    EntityMelonBomb entityMelon = new EntityMelonBomb(par2World, (double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F));
                    entityMelon.rotationYaw = (float)(((MathHelper.floor_double((double)(par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!par2World.getCollidingBoundingBoxes(entityMelon, entityMelon.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return par1ItemStack;
                    }
